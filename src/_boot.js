...
process.on("uncaughtException", e => {
    signale.fatal(e);
    dialog.showErrorBox("ArcticDEX crashed", e.message || "Cannot retrieve error message.");
    if (tty) {
        tty.close();
    }
    if (extraTtys) {
...
signale.start(`Starting ArcticDEX v${app.getVersion()}`);
signale.info(`With Node ${process.versions.node} and Electron ${process.versions.electron}`);
signale.info(`Renderer is Chrome ${process.versions.chrome}`);

const gotLock = app.requestSingleInstanceLock();
if (!gotLock) {
    signale.fatal("Error: Another instance of ArcticDEX is already running. Cannot proceed.");
    app.exit(1);
}
